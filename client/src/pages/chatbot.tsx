  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <p className="text-xs text-slate-500 font-medium">
        {isThinking ? "PaperBot is calculating…" : hasResult ? "Here's your estimate" : "Fill the form & ask me!"}
      </p>
    </div>
  );
}
...
export default function ChatbotPage() {
  ...
  useEffect(() => {
      ...
      mainEntity: [
        {
          ...
          acceptedAnswer: { "@type": "Answer", text: "Under 2 seconds with PaperBot's AI quote engine." },
        },
      ],
    };
    ...
    return () => { document.getElementById("paperbot-faq-schema")?.remove(); };
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-teal-50 py-10 px-4">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-4xl font-bold tracking-tight text-slate-900">PaperBot</h1>
          <p className="mt-2 text-slate-500">Notary, Construction, and Labor & Payment quotes in seconds.</p>
        </div>

        {/* PaperBot mascot — always visible at top */}
        <div className="flex justify-center mb-6">
          <PaperBot action={mascotAction} isThinking={quoteMutation.isPending} hasResult={hasResult} />
        </div>
        ...
        {quoteMutation.isError && (
          <div className="mb-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
            PaperBot couldn't connect right now. Please try again or{" "}
        ...
        {!hasResult && (
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6 space-y-5">
            ...
            <Button
              ...
              data-testid="btn-ask-paperbot"
            >
              {quoteMutation.isPending ? (
                <span className="flex items-center gap-2">
                  <RefreshCw className="w-4 h-4 animate-spin" /> PaperBot is thinking…
                </span>
              ) : (
                <span className="flex items-center gap-2">
                  <Sparkles className="w-4 h-4" /> Ask PaperBot
                </span>
              )}
            </Button>
