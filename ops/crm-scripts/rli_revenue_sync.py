...
# Texas notary bonds carry a $21 statutory Secretary-of-State filing fee, and
# ~1 in 3 orders add an optional E&O rider on top, neither of which the RLI
# weekly statement's TotalPremium includes (it is the carrier's own premium
# only). RLI's live portal — and bk_bonds, which mirrors it exactly — legitimately
# show the full retail total. Confirmed 2026-09-23 against the whole book: every
# $50 (no-E&O) notary bond is EXACTLY $71 bk-side; the gap otherwise runs $16
# (a Rider amendment, which does not repeat the full filing fee) to $38 (higher
# E&O tiers). This is not a data bug — see CLAUDE.md and get-bond.tsx's pricing
# comment block for the full writeup. A gap outside that band, or bk at or below
# the carrier's own premium, is still a real divergence and stays flagged.
NOTARY_FILING_FEE = Decimal("21.00")
NOTARY_GAP_CEILING = Decimal("60.00")


def _field_div(name, xls_val, bk_val, bond_type=None, status_detail=None):
    """Return a divergence detail string, or None if within tolerance.
    ...
    if x is None:
        return None
    if b is None:
        return f"{name}: xls={x} bk=NULL"
    if name == "premium" and bond_type == "notary":
        gap = b - x
        is_rider = bool(status_detail) and "rider" in status_detail.lower()
        floor = Decimal("0.00") if is_rider else NOTARY_FILING_FEE
        if gap < floor or gap > NOTARY_GAP_CEILING:
            return (f"{name}: xls={x} bk={b} (notary fee/E&O structure expected "
                     f"bk-xls in [{floor}, {NOTARY_GAP_CEILING}], got {gap})")
        return None
    if (x - b).copy_abs() > CENT:
        return f"{name}: xls={x} bk={b}"
    return None
...
def reconcile_bk_bonds(cur):
    ...
    for (bond_no, r_prem, r_comm, r_amt,
         bk_no, b_prem, b_comm, b_amt, b_type, b_status_detail) in rows:
        if bk_no is None:
            counts["missing_in_bk_bonds"] += 1
            flagged.append((bond_no, "missing_in_bk_bonds",
                            "bond in RLI weekly XLS but absent from bk_bonds"))
            continue
        d = _field_div("premium", r_prem, b_prem, bond_type=b_type,
                        status_detail=b_status_detail)
        if d:
            counts["premium"] += 1
