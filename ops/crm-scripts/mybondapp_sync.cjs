...
// 519 portal items at 10 per page is 52 pages. The old default of 40 would have
// truncated the list the moment pagination started advancing again -- swapping one
// silent shortfall for another. Headroom, not a tight fit.
const MAX_PAGES    = Math.max(1, parseInt(process.env.RLI_MAX_PAGES || '80', 10) || 80);

/**
 * Resolve once the bond grid has genuinely re-rendered; false on timeout.
 *
 * WHY: the old loop clicked next, slept 4s, then called waitForNetworkIdle.
 * waitForNetworkIdle returns IMMEDIATELY when the network is already quiet, which is
 * precisely the state in the moments after a click and before its XHR leaves. So the
 * loop kept reading the previous page's DOM, saw an unchanged first bond, and decided
 * pagination was stuck. On 2026-08-29 that scraped 20 of 519 items and alarmed.
 *
 * Poll the portal's own "11 - 20 of 519 Items" line instead: it changes on every
 * page, so it is a true advance signal rather than a guess about timing.
 */
async function waitForGridAdvance(page, beforeMarker, timeout = 25000) {
  if (!beforeMarker) {
    // No marker to compare against -- fall back to the old timing behaviour rather
    // than blocking the run outright.
  ...
  return page.waitForFunction(
    (prev) => {
      const m = document.body.innerText.match(/\d+\s*-\s*\d+ of \d+ Items/i);
      return !!m && m[0] !== prev;
    },
    { timeout, polling: 400 },
    beforeMarker
...
async function ensureSession(page) {
  ...
  // animates in — always type with length verification and retry. (Production
  // fix from the deployed 2026 script; a bare page.type() leaves a truncated
  // username and the login silently stalls on the sign-in screen.)
...
async function scrapeAllBonds(bondPage, stats) {
  ...
  while (true) {
    ...
    // Snapshot the range line BEFORE clicking; waitForGridAdvance compares against it.
    const beforeMarker = await bondPage.evaluate(() => {
      const m = document.body.innerText.match(/\d+\s*-\s*\d+ of \d+ Items/i);
      return m ? m[0] : '';
    }).catch(() => '');
    ...
    if (!(await waitForGridAdvance(bondPage, beforeMarker))) {
      // A dropped click is far likelier than a genuinely stuck grid, so retry once
      // before declaring the list incomplete.
      console.log('  \u26a0 Grid did not advance \u2014 re-clicking next once');
      const reclicked = await bondPage.evaluate(() => {
        const a = Array.from(document.querySelectorAll('a'))
          .find(x => ['\u203a', '\u00bb', 'Next'].includes(x.textContent.trim()));
        if (!a) return false;
      ...
      if (!reclicked || !(await waitForGridAdvance(bondPage, beforeMarker))) {
        console.log(`  \u26a0 Grid still reads "${beforeMarker}" after a retry \u2014 stopping.`);
        stats.paginationRepeat = true;
        break;
      }
    }
...
(async () => {
  ...
  try {
    ...
    // Pick whichever tab is actually ON the bond list. The My Bond Center
    // click can open a NEW tab that lands on /Agency (the "Select a Bond"
    // product page) while the fallback path drives the ORIGINAL tab to
    // /Agency/BondList — blindly preferring the new tab then scrapes the
    // product catalog and finds zero bonds (observed 2026-08-24).
    const onBondList = p => { try { return p.url().includes('/Agency/BondList'); } catch (_) { return false; } };
    let bondPage = newTabPage || page;
    if (!onBondList(bondPage)) {
      if (onBondList(page)) {
        bondPage = page;
        console.log('  → New tab is not on BondList — using original tab');
      } else if (bondPage.url().includes('rlisurety.rlicorp.com')) {
        console.log('  → Navigating active tab to BondList directly...');
    ...
    if (!bondUrl.includes('/Agency/BondList')) {
      throw new Error(`Bond page is not the BondList: ${bondUrl}`);
    }
  ...
  // Drift ratio counts only STRUCTURAL parse failures. Abandoned/withdrawn
  // applications legitimately have no bond number (the old script dropped
  // ~130 of them silently every night); they are still listed in the skipped
  // report for visibility but are a known portal state, not parser drift.
  const KNOWN_SKIP_REASONS = new Set(['no-bond-number-non-saved']);
  const structuralSkips = stats.skipped.filter(s => !KNOWN_SKIP_REASONS.has(s.reason));
  const knownSkips = stats.skipped.length - structuralSkips.length;
  if (knownSkips) console.log(`  (of the skipped, ${knownSkips} are known-category: abandoned/no-bond-number — not counted toward drift)`);
  if (totalCards > 0 && structuralSkips.length / totalCards > 0.10) {
    driftAlarms.push(`Skipped ${structuralSkips.length}/${totalCards} cards structurally (>10%) — parser drift suspected. See skipped report.`);
  }
