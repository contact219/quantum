...
export default function CountyBondWatch() {
  ...
  const title = `${countyName} County Contractor License Expirations | Texas Bond Watch`;
  const description = c
    ? `${countyName} County: ${c.expired.toLocaleString()} TDLR contractor licenses (${expiredPct}%) are past their expiration date and ${c.expiring_30d.toLocaleString()} more expire in 30 days. Look up any contractor's license free.`
    : `TDLR contractor license expirations in ${countyName} County, Texas, from public TDLR data refreshed monthly.`;
      ...
      <section style={{ background: "linear-gradient(135deg,#0a0f1e 0%,#111827 100%)", padding: "52px 24px 44px", textAlign: "center" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <h1 style={{ fontSize: "clamp(26px,5vw,46px)", fontWeight: 900, color: "#fff", lineHeight: 1.1, margin: "0 0 16px" }}>
            {countyName} County<br />Contractor License Tracker
          </h1>
          <p style={{ fontSize: 16, color: "#94a3b8", maxWidth: 540, margin: "0 auto 32px", lineHeight: 1.6 }}>
            TDLR license expiration data for contractors in {countyName} County, refreshed monthly.
          ...
          {c ? (
            <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap", marginBottom: 24 }}>
              {([
                ...
                { label: "Expired Licenses", value: (c.expired as number).toLocaleString(), color: "#dc2626" },
                { label: "Expiring 30 Days", value: (c.expiring_30d as number).toLocaleString(), color: "#d97706" },
                { label: "Expired Share", value: `${expiredPct}%`, color: "#94a3b8" },
      ...
      <section style={{ background: "#161b22", padding: "40px 24px", borderTop: "1px solid #21262d" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 8 }}>
            Embed {countyName} County License Stats on Your Website
          </h2>
          <p style={{ fontSize: 13, color: "#64748b", marginBottom: 16 }}>
            HOA boards, permit offices, and property managers can add {countyName} County contractor license data to their website with one line of code:
          </p>
          ...
          <p style={{ fontSize: 11, color: "#475569", marginTop: 8 }}>Click to copy · TDLR public data · Refreshed monthly · Free</p>
        </div>
      </section>
      ...
      <section style={{ background: "#0d1117", padding: "48px 24px", borderTop: "1px solid #21262d" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 20 }}>
            What to Check Before Hiring in {countyName} County
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 16 }}>
            {[
              { icon: "📋", t: "License Status", b: "Check that the contractor's TDLR license is active and has not expired." },
              { icon: "🔒", t: "Insurance", b: "TDLR requires licensed contractors to carry liability insurance. Ask for a current certificate of insurance." },
              { icon: "⚖️", t: "Right License Type", b: "Make sure the license type matches the work, for example an electrical contractor license for electrical work." },
              { icon: "🛡️", t: "Paperwork", b: "Get a written contract and avoid paying the full amount upfront." },
            ].map(item => (
      ...
      <section style={{ background: "#0a0f1e", padding: "48px 24px", borderTop: "1px solid #1e293b", textAlign: "center" }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 14, color: "#94a3b8", marginBottom: 24, lineHeight: 1.6 }}>
            TDLR licenses require liability insurance, not a surety bond. If a city or a project owner asks you for a bond, we can write it, from $100.
          </p>
          <Link href="/quote">
